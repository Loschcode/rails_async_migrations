...
RSpec.describe(AsyncSchemaMigration, type: :model) do
  ...
  describe "Associations" do
    ...
    it {
 is_expected.to(validate_inclusion_of(:state).in_array(["created", "pending", "processing", "done", "failed"])) }
    it { is_expected.to(validate_inclusion_of(:direction).in_array(["up", "down"])) }
  end
